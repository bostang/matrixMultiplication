...
for k in range(len(algo1)):
    algo1[k] = float(algo1[k])

    # menyimpan run-time algoritma 2 ke array algo2
algo2 = fp.readline().split(',')
for k in range(len(algo2)):
    algo2[k] = float(algo2[k])

n_axis = ['10','100'] # dibuat dalam character, supaya ketika di-gambar plot tidak ada nilai antara (200,300, dst.) 
...
plt.title("Grafik run-time program terhadap jumlah input ($t$ vs $n$) Algoritma Perkalian Matriks")
